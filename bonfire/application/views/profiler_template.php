<style type="text/css">
	#codeigniter-profiler { clear: both; background: #222; padding: 0 5px; font-family: Helvetica, sans-serif; font-size: 10px !important; line-height: 12px; position: absolute; width: auto; min-width: 66em; max-width: 90%; z-index: 1000; display: none; }
	...
	#ci-profiler-menu a:link, #ci-profiler-menu a:visited { display: inline-block; padding: 7px 0; margin: 0; color: #ccc; text-decoration: none; font-weight: lighter; cursor: pointer; text-align: center; width: 13.4%; border-bottom: 4px solid #444; }
	...
	#ci-profiler-menu-exposed span, #ci-profiler-exposed h2 { color: #00D28C}
	...
	#codeigniter-profiler table.main td { padding: 0px 15px 7px 15px; margin:4px; text-align: left; vertical-align: top; color: #aaa; border-bottom: 1px dotted #444; line-height: 1.5; background: #101010 !important; font-size: 12px !important; }
	#codeigniter-profiler table.main td td:last-child tr td {border-bottom: 1px dotted transparent;}
	...
	#ci-profiler-exposed .keys-list {margin-left: 0;}
	#ci-profiler-exposed .keys-list li{margin-right: 15px;display: inline;}
	#ci-profiler-exposed .fill{width:100%; text-align: left;}
	#ci-profiler-exposed .deep td {min-width: 8em;}
	#ci-profiler-exposed .hidden {display: none; !important}
</style>
...
<div id="codeigniter-profiler" class="bottom-left">
	...
	<div id="ci-profiler-menu">
		...
		<!-- Exposed Data -->
		<?php if (isset($sections['exposed'])) : ?>
			<a href="#" id="ci-profiler-menu-exposed" onclick="ci_profiler_bar.show('ci-profiler-exposed', 'ci-profiler-menu-exposed'); return false;">
				<span>Exposed</span> Data
			</a>
		<?php endif; ?>
...
<?php if (count($sections) > 0) : ?>
	...
	<?php if (isset($sections['exposed'])) :?>
		...
		<div id="ci-profiler-exposed" class="ci-profiler-box" style="display: none">
	...
	<ul class="keys-list">
		<?php foreach ($sections['exposed'] as $key => $value): ?>
			<li><?php echo "&bull; {$key} (".gettype($value).")";?></li>
		<?php endforeach; ?>
	</ul>
	<hr>
	<?php
	// sort exposed variables into types to handle differently
	$object_array = array();
	$array_vars = array();
	$normal_vars = array();
	foreach ($sections['exposed'] as $key => $value) {
		if (is_array($value)) {
			$has_array = false;
	...
	<!-- primatives -->
	<table class="main">
	<?php foreach ($normal_vars as $key => $value) : ?>
		<tr><td class="hilight">
			<?php echo $key; ?>
		</td>
		<td class="fill">
	...
	<!-- objects -->
	<?php foreach ($array_vars as $name => $object): ?>
	<a href="#" onclick="$('#codeigniter-profiler #<?php echo $name ?>_table').toggle(); return false;">
		<h2><?php echo $name; ?></h2><b class='caret'></b>
	</a>
	<table class="main deep" id="<?php echo $name; ?>_table">
								...
								<tr>
									...
									<td class="fill">
										<?php if (is_array($v) || is_object($v)): ?>
											<?php foreach ($v as $innerk => $innerv): ?>
												<table>
													<tr>
														<td class="hilight">
															<?php echo $innerk ?>
														...
														<td class="fill">
															<?php if (is_array($innerv) || is_object($innerv)): ?>
																<?php echo htmlentities(json_encode($innerv)) ?>
															<?php else: ?>
																<?php echo htmlentities($innerv) ?>
															<?php endif ?>
														</td>
													</tr>
												</table>
	...
	<!-- object arrays -->
	<?php foreach ($object_array as $name => $object): ?>
	<a id="<?php echo $name; ?>_table_toggle" href="#" onclick="$('#codeigniter-profiler #<?php echo $name ?>_table').toggle(); return false;">
		<h2 style="display:inline;"><?php echo $name; ?></h2>
	</a>
	<table class="main" id="<?php echo $name; ?>_table">
		<tr class="hilight">
		...
		<tr>
			<?php foreach ($object as $row): ?>
				<?php foreach ($row as $field => $value): ?>
					<td>
						<?php if (is_array($value) || is_object($value)): ?>
							<pre><?php echo htmlentities(print_r($value), TRUE); ?></pre>
						<?php else: ?>
							<?php echo htmlentities($value); ?>
						<?php endif ?>
					</td>
...
<!-- add exposed data back into profiler bar -->
